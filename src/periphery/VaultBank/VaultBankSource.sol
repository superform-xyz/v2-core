...
abstract contract VaultBankSource is IVaultBankSource {
    ...
    //////////////////////////////////////////////////////////////*/
    // ------------------ SYNTHETIC ASSETS ------------------
    function _lockAssetForChain(address account, address token, uint256 amount, uint64 toChainId, uint256 nonce)
        internal
    {
    ...
    function _releaseAssetFromChain(address account, address token, uint256 amount, uint64 fromChainId, uint256 nonce)
        internal
    {
        if (account == address(0)) revert INVALID_ACCOUNT();
        if (token == address(0)) revert INVALID_TOKEN();

...
contract StargateV2SendHook is BaseHook, ISuperHookContextAware {
        ...
        returns (Execution[] memory executions)
    {
        ...
        // @dev `value` is not adjusted here; for native OFT pools the SuperBundler must ensure `value` already
        //      accounts for the expected outAmount from the previous hook (value = messaging fee, not bridged amount)

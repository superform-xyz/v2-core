...
/// @title RelaySendFundsAndExecuteOnDstHook
/// @author Superform Labs
/// @dev Bridges tokens via the Relay Protocol depository. Relay has no destination receiver-callback:
///      the origin deposit carries only the order `id`; the destination execution (including the signed
///      intent payload) is composed off-chain by the SuperBundler in the Relay quote's txs[] and executed
///      atomically by the Relay solver. Consequently this hook appends no destination message on-chain.
/// @dev For ERC20 deposits the depository pulls funds via transferFrom, so a prior approval to the
///      depository is required — chain an approve hook or use ApproveAndRelaySendFundsAndExecuteOnDstHook.
/// @dev token == address(0) selects the native ETH path (depositNative with value == amount).
/// @dev The depositor passed to the depository is ALWAYS the account, never taken from hook data,
///      pinning Relay's off-chain credit/refund attribution to the smart account.
...
contract RelaySendFundsAndExecuteOnDstHook is
    BaseHook,
    ISuperHookContextAware,
    ISuperHookInflowOutflow,
    ISuperHookOutflow
{
    ...
    function description() external pure override returns (string memory) {
        return "Bridges tokens via the Relay depository with destination execution by Relay solvers";
    }

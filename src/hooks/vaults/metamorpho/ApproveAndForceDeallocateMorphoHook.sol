...
/// @notice NONACCOUNTING hook with token approval lifecycle around Morpho Vault V2's forceDeallocate()
...
/// @notice         address adapter = BytesLib.toAddress(data, 72);
/// @notice         uint256 assets = BytesLib.toUint256(data, 92);
/// @notice         uint256 deadline = BytesLib.toUint256(data, 124);
/// @notice         uint256 maxPenaltyBps = BytesLib.toUint256(data, 156);
/// @notice         bool usePrevHookAmount = _decodeBool(data, 188);
/// @notice         bytes adapterData = BytesLib.slice(data, 189, data.length - 189);
...
contract ApproveAndForceDeallocateMorphoHook is BaseHook, ISuperHookContextAware {
        ...
        returns (Execution[] memory executions)
    {
        ...
        // Deadline check (0 = no deadline) — inline to avoid extra stack slot
        {
        ...
        // Build executions: zero-approve-action-zero
        executions = new Execution[](4);
        executions[0] =
            Execution({ target: token, value: 0, callData: abi.encodeCall(IERC20.approve, (vault, 0)) });

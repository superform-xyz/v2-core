...
/// @title BurnSuperPositionsHook
/// @author Superform Labs
/// @notice Burns SuperPosition tokens from VaultBank.
/// @dev data has the following structure:
///         bytes32 yieldSourceOracleId = bytes32(BytesLib.slice(data, 0, 32), 0);
///         address superPosition = BytesLib.toAddress(data, 32);
...
contract BurnSuperPositionsHook is BaseHook, VaultBankLockableHook, ISuperHookInflowOutflow, ISuperHookContextAware {
        ...
        returns (Execution[] memory executions)
    {
        ...
        // - The user is independently burning shares.
        //
        // Note: It is not possible to distinguish whether the shares originate from an asset directly
        // or from a yield source.
        address superPosition = data.extractYieldSource();
        address vaultBank = BytesLib.toAddress(data, 85);
        uint256 dstChainId = BytesLib.toUint256(data, 105);
        bool usePrevHookAmount = _decodeBool(data, USE_PREV_HOOK_AMOUNT_POSITION);

        if (vaultBank == address(0) || superPosition == address(0)) revert ADDRESS_NOT_VALID();
        ...
        executions[2] = Execution({
            ...
            callData: abi.encodeCall(
                IVaultBank.burnSuperPosition, (amount, superPosition, dstChainId.toUint64(), yieldSourceOracleId)
            )
        });
        executions[3] =
            Execution({ target: superPosition, value: 0, callData: abi.encodeCall(IERC20.approve, (vaultBank, 0)) });

...
/// @title SwapSparkPSMExactOutHook
/// @author Superform Labs
/// @notice Hook for executing exact-output swaps via Spark PSM (assumes pre-approved)
/// @dev data has the following structure:
/// @notice         address assetIn = BytesLib.toAddress(data, 0);
/// @notice         address assetOut = BytesLib.toAddress(data, 20);
/// @notice         uint256 amountOut = BytesLib.toUint256(data, 40);
/// @notice         uint256 maxAmountIn = BytesLib.toUint256(data, 72);
...
contract SwapSparkPSMExactOutHook is BaseHook, ISuperHookContextAware, ISuperHookInflowOutflow, ISuperHookOutflow {
    ...
    /// @notice Initialize the Spark PSM ExactOut swap hook
    ...
    function name() external pure override returns (string memory) {
        return "Swap Spark PSM Exact Out";
    }

    /// @notice One-sentence description of what this hook does
    function description() external pure override returns (string memory) {
        return "Swaps for exact output tokens via Spark PSM";
    }
        ...
        returns (Execution[] memory executions)
    {
        ...
        uint256 amountOut = data.toUint256(40);
        uint256 maxAmountIn = data.toUint256(72);

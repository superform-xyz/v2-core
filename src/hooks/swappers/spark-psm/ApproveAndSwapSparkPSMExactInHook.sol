...
contract ApproveAndSwapSparkPSMExactInHook is
    BaseHook,
    ISuperHookSwap,
    ISuperHookContextAware,
    ISuperHookInflowOutflow,
    ISuperHookOutflow
{
        ...
        returns (Execution[] memory executions)
    {
        ...
        // approve(0) -> approve(inputAmount) -> swap -> approve(0)
        ...
        // Set approval to exact inputAmount
        executions[1] = Execution({
            target: inputToken,
            value: 0,
            callData: abi.encodeCall(IERC20.approve, (address(PSM), inputAmount))
        });

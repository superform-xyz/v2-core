...
library ZeroExTransactionPatcher {
    ...
    function _patchSettlerCalldata(
        ...
        uint256 oldAmount,
        uint256 newAmount
    ) private pure returns (bytes memory patchedCalldata) {
    ...
    function _patchActionsArray(
        ...
        uint256 oldAmount,
        uint256 newAmount
    ) private pure returns (bytes[] memory patchedActions) {
        patchedActions = new bytes[](actions.length);
    ...
    function _patchSingleAction(
        ...
        uint256 oldAmount,
        uint256 newAmount
    ) private pure returns (bytes memory patchedAction) {
    ...
    function _patchBasicAction(
        ...
        if (actionData.length < 164) { // 4 + 32*5 = minimum size for BASIC action
            return actionData;
        }
        ...
        patchedAction = abi.encodeWithSelector(
            BASIC_SELECTOR,
            sellToken,
            newBps,
            pool,
    ...
    /// @dev UNISWAPV2(address recipient, address sellToken, uint256 bps, address pool, uint24 swapInfo, uint256 amountOutMin)
    function _patchUniswapV2Action(
        bytes memory actionData,
        uint256 oldAmount,
        uint256 newAmount
    ) private pure returns (bytes memory patchedAction) {
        if (actionData.length < 196) { // 4 + 32*6 = minimum size for UNISWAPV2 action
            return actionData;
        }
    ...
    function _patchUniswapV3Action(
        ...
        if (actionData.length < 132) { // 4 + 32*4 = minimum size for UNISWAPV3 action
            return actionData;
        }

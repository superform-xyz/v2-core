    ...
    ISuperHookInspector
{
        ...
        returns (Execution[] memory executions)
    {
        ...
        executions[2] =
            Execution({ target: vaultBank, value: 0, callData: abi.encodeCall(IVaultBank.lockAsset, (yieldSourceOracleId, account, spToken, address(this), amount, dstChainId.toUint64())) });
        executions[3] =
            Execution({ target: spToken, value: 0, callData: abi.encodeCall(IERC20.approve, (vaultBank, 0)) });
    }
    ...
    function inspect(bytes calldata data) external pure returns (bytes memory) {
        return abi.encodePacked(data.extractYieldSource() /**spToken*/, BytesLib.toAddress(data, 85) /**vaultBank*/);
    }

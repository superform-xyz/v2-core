...
2. **Debt oracle shape = Euler** (asset-denominated): identity PPS, pure identity converters, `getBalanceOfOwner = getTVLByOwnerOfShares = drawn + premium` (matches `BaseAaveV4LoanHookV2._totalDebt`), plus **Morpho-style `getAssetOutputWithFees` override** and the full Euler NatSpec fee-invariant block. Supply oracle: same identity shape, ~~fee-capable (no override)~~ **[SUPERSEDED by PR #997 review F1 — the supply oracle now bypasses the fee view exactly like the debt oracle; feePercent = 0 is an operational invariant for BOTH oracle ids]**, `getBalanceOfOwner = getUserSuppliedAssets`; **TVL source open** (minimal spoke interface lacks aggregate getter — extend `IAaveV4Spoke` after Joao call).

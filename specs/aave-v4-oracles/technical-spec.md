...
- [x] **ONE reserve key, TWO oracles — the aggregation surface.** The registry derives a single pseudo-address per `(spoke, reserveId)` and BOTH oracles accept it, so on a lending reserve the same `(key, user)` legitimately yields a supply figure AND a debt figure with opposite economic sign (measured live on Base: the same USDC key reports both legs of one borrower). Two consequences, both pinned by `test/integration/oracles/AaveV4BaseEquitiesE2EFork.t.sol`:
  - **Consumer side:** net within a reserve, never add; and never add across reserves whose underlyings differ (8-decimal equities vs 6-decimal USDC). Reserve-level `getTVL` is likewise NOT additive across the two oracles — debt is drawn out of the pool the supply oracle reports, so summing counts the borrowed portion twice.
  - **Ledger side (the real double count):** `BaseLedger` accumulators are keyed `(user, yieldSource)` with NO `yieldSourceOracleId` component, so registering BOTH oracle ids against the SAME ledger makes the supply and debt legs of one reserve key sum into a single accumulator slot. **Operational invariant for the future wiring phase: never register both oracle ids against the same ledger for the same reserve key** — use one ledger per side, or register only the side being accounted. Inert today (loan hooks are NONACCOUNTING); demonstrated and mitigated by executable tests, the same shape as the T1 feePercent demonstration.

...
- **No `getAssetOutputWithFees` override** — inherited fee-capable path retained (interview decision #6). NatSpec must state: (a) with identity PPS, ledger cost-basis math tracks principal, so configured fees under-charge yield and never over-charge; (b) missing/invalid ledger config falls through to plain output (inherited try/catch — expected for markets with no fee config); (c) real yield-fee capability requires future hook wiring (S1) and possibly a shares-PPS oracle variant (documented non-goal).

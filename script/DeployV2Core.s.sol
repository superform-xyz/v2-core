...
contract DeployV2Core is DeployV2Base, ConfigCore {
        ...
        returns (ContractAvailability memory availability)
    {
        ...
        string[] memory potentialSkips = new string[](47);
        uint256 skipCount = 0;
        // Adapter contracts (6 contracts - conditionally deployed)
        string[6] memory adapterContracts =
            ["AcrossV3AdapterV2", "RelayAdapter", "CCTPAdapter", "DebridgeAdapter", "StargateAdapter", "StargateAdapterV2"];
        ...
        internal
    {
        ...
        if (availability.pendleRouterHooks) {
            ...
            __checkContract(
                PENDLE_PT_HOOK_KEY,
                __getSalt(PENDLE_PT_HOOK_KEY),
                abi.encode(configuration.pendleRouters[chainId]),
                env
            );
        } else {
            console2.log(
                "SKIPPED PendleRouterSwapHook, PendleRouterRedeemHook, PendleUnifiedHook & PendlePTHook: Pendle Router not configured for chain",
                chainId
            );
        }
        ...
        returns (ContractVerification[] memory contracts)
    {
        ...
        contracts[12] = ContractVerification({
            ...
            bytecodePath: __getBytecodeArtifactPath("EulerDebtOracle", env),
            constructorArgs: ""
        });

...
contract DeployV2Core is DeployV2Base, ConfigCore {
    ...
    function _deployHooks(uint64 chainId, uint256 env) private returns (HookAddresses memory hookAddresses) {
        ...
        if (availability.swapOdosV3Hooks) {
            ...
            hooks[62] = HookDeployment("", "", ""); // Empty deployment
            hooks[63] = HookDeployment("", "", ""); // Empty deployment
        }
        ...
        hookAddresses.swapOdosV3Hook =
            Strings.equal(hooks[62].name, SWAP_ODOSV3_HOOK_KEY) ? addresses[62] : address(0);
        hookAddresses.approveAndSwapOdosV3Hook =
            Strings.equal(hooks[63].name, APPROVE_AND_SWAP_ODOSV3_HOOK_KEY) ? addresses[63] : address(0);

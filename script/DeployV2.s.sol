...
contract DeployV2 is Script, Configuration {
    ...
    {
        uint256 len = 47; // Updated length including Pendle & batchTransferFrom hooks
        ...
        hooks[24] = HookDeployment(
            ...
            abi.encodePacked(
                type(DeBridgeCancelOrderHook).creationCode,
                abi.encode(DEBRIDGE_DLN_SRC)
            )
        );

        hooks[25] = HookDeployment(FLUID_CLAIM_REWARD_HOOK_KEY, type(FluidClaimRewardHook).creationCode);
        hooks[26] = HookDeployment(FLUID_STAKE_HOOK_KEY, type(FluidStakeHook).creationCode);
        hooks[27] = HookDeployment(APPROVE_AND_FLUID_STAKE_HOOK_KEY, type(ApproveAndFluidStakeHook).creationCode);
        hooks[28] = HookDeployment(FLUID_UNSTAKE_HOOK_KEY, type(FluidUnstakeHook).creationCode);
        hooks[29] = HookDeployment(GEARBOX_CLAIM_REWARD_HOOK_KEY, type(GearboxClaimRewardHook).creationCode);
        hooks[30] = HookDeployment(GEARBOX_STAKE_HOOK_KEY, type(GearboxStakeHook).creationCode);
        hooks[31] = HookDeployment(GEARBOX_APPROVE_AND_STAKE_HOOK_KEY, type(ApproveAndGearboxStakeHook).creationCode);
        hooks[32] = HookDeployment(GEARBOX_UNSTAKE_HOOK_KEY, type(GearboxUnstakeHook).creationCode);
        hooks[33] = HookDeployment(YEARN_CLAIM_ONE_REWARD_HOOK_KEY, type(YearnClaimOneRewardHook).creationCode);
        hooks[34] = HookDeployment(ETHENA_COOLDOWN_SHARES_HOOK_KEY, type(EthenaCooldownSharesHook).creationCode);
        hooks[35] = HookDeployment(ETHENA_UNSTAKE_HOOK_KEY, type(EthenaUnstakeHook).creationCode);
        ...
        hooks[40] = HookDeployment(CANCEL_REDEEM_REQUEST_7540_HOOK_KEY, type(CancelRedeemRequest7540Hook).creationCode);
        ...
        hooks[43] = HookDeployment(CANCEL_REDEEM_HOOK_KEY, type(CancelRedeemHook).creationCode);
        ...
        hookAddresses.approveAndWithdraw7540VaultHook =
            Strings.equal(hooks[17].name, APPROVE_AND_WITHDRAW_7540_VAULT_HOOK_KEY) ? addresses[17] : address(0);
        hookAddresses.approveAndRedeem7540VaultHook =
            Strings.equal(hooks[18].name, APPROVE_AND_REDEEM_7540_VAULT_HOOK_KEY) ? addresses[18] : address(0);
        ...
        hookAddresses.fluidClaimRewardHook =
            Strings.equal(hooks[25].name, FLUID_CLAIM_REWARD_HOOK_KEY) ? addresses[25] : address(0);
        hookAddresses.fluidStakeHook = Strings.equal(hooks[26].name, FLUID_STAKE_HOOK_KEY) ? addresses[26] : address(0);
        hookAddresses.approveAndFluidStakeHook =
            Strings.equal(hooks[27].name, APPROVE_AND_FLUID_STAKE_HOOK_KEY) ? addresses[27] : address(0);
        hookAddresses.fluidUnstakeHook =
            Strings.equal(hooks[28].name, FLUID_UNSTAKE_HOOK_KEY) ? addresses[28] : address(0);
        hookAddresses.gearboxClaimRewardHook =
            Strings.equal(hooks[29].name, GEARBOX_CLAIM_REWARD_HOOK_KEY) ? addresses[29] : address(0);
        hookAddresses.gearboxStakeHook =
            Strings.equal(hooks[30].name, GEARBOX_STAKE_HOOK_KEY) ? addresses[30] : address(0);
        hookAddresses.approveAndGearboxStakeHook =
            Strings.equal(hooks[31].name, GEARBOX_APPROVE_AND_STAKE_HOOK_KEY) ? addresses[31] : address(0);
        hookAddresses.gearboxUnstakeHook =
            Strings.equal(hooks[32].name, GEARBOX_UNSTAKE_HOOK_KEY) ? addresses[32] : address(0);
        hookAddresses.yearnClaimOneRewardHook =
            Strings.equal(hooks[33].name, YEARN_CLAIM_ONE_REWARD_HOOK_KEY) ? addresses[33] : address(0);
        hookAddresses.ethenaCooldownSharesHook =
            Strings.equal(hooks[34].name, ETHENA_COOLDOWN_SHARES_HOOK_KEY) ? addresses[34] : address(0);
        hookAddresses.ethenaUnstakeHook =
            Strings.equal(hooks[35].name, ETHENA_UNSTAKE_HOOK_KEY) ? addresses[35] : address(0);
        hookAddresses.spectraExchangeHook =
            Strings.equal(hooks[36].name, SPECTRA_EXCHANGE_HOOK_KEY) ? addresses[36] : address(0);
        hookAddresses.pendleRouterSwapHook =
            Strings.equal(hooks[37].name, PENDLE_ROUTER_SWAP_HOOK_KEY) ? addresses[37] : address(0);
        hookAddresses.pendleRouterRedeemHook =
            Strings.equal(hooks[38].name, PENDLE_ROUTER_REDEEM_HOOK_KEY) ? addresses[38] : address(0);
        hookAddresses.cancelDepositRequest7540Hook =
            Strings.equal(hooks[39].name, CANCEL_DEPOSIT_REQUEST_7540_HOOK_KEY) ? addresses[39] : address(0);
        hookAddresses.cancelRedeemRequest7540Hook =
            Strings.equal(hooks[40].name, CANCEL_REDEEM_REQUEST_7540_HOOK_KEY) ? addresses[40] : address(0);
        hookAddresses.claimCancelDepositRequest7540Hook =
            Strings.equal(hooks[41].name, CLAIM_CANCEL_DEPOSIT_REQUEST_7540_HOOK_KEY) ? addresses[41] : address(0);
        hookAddresses.claimCancelRedeemRequest7540Hook =
            Strings.equal(hooks[42].name, CLAIM_CANCEL_REDEEM_REQUEST_7540_HOOK_KEY) ? addresses[42] : address(0);
        hookAddresses.cancelRedeemHook =
            Strings.equal(hooks[43].name, CANCEL_REDEEM_HOOK_KEY) ? addresses[43] : address(0);
        hookAddresses.morphoBorrowHook =
            Strings.equal(hooks[44].name, MORPHO_BORROW_HOOK_KEY) ? addresses[44] : address(0);
        hookAddresses.morphoRepayHook =
            Strings.equal(hooks[45].name, MORPHO_REPAY_HOOK_KEY) ? addresses[45] : address(0);
        hookAddresses.morphoRepayAndWithdrawHook =
            Strings.equal(hooks[46].name, MORPHO_REPAY_AND_WITHDRAW_HOOK_KEY) ? addresses[46] : address(0);

...
contract DeployV2 is Script, Configuration {
    ...
    {
        uint256 len = 44; // Updated length including Pendle & batchTransferFrom hooks
        ...
        hookAddresses.approveAndWithdraw7540VaultHook =
            Strings.equal(hooks[15].name, APPROVE_AND_WITHDRAW_7540_VAULT_HOOK_KEY) ? addresses[16] : address(0);
        hookAddresses.approveAndRedeem7540VaultHook =
            Strings.equal(hooks[16].name, APPROVE_AND_REDEEM_7540_VAULT_HOOK_KEY) ? addresses[17] : address(0);

...
contract DeployV2 is Script, Configuration {
    ...
    {
        uint256 len = 46; // Updated length including Pendle, batchTransferFrom & MorphoBorrow hooks
        ...
        hooks[13] = HookDeployment(APPROVE_AND_REQUEST_REDEEM_7540_VAULT_HOOK_KEY, type(ApproveAndRequestRedeem7540VaultHook).creationCode);
        ...
        hookAddresses.approveAndRequestRedeem7540VaultHook =
            Strings.equal(hooks[13].name, APPROVE_AND_REQUEST_REDEEM_7540_VAULT_HOOK_KEY) ? addresses[13] : address(0);
        hookAddresses.redeem7540VaultHook =
            Strings.equal(hooks[14].name, REDEEM_7540_VAULT_HOOK_KEY) ? addresses[14] : address(0);

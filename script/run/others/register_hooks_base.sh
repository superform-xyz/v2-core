#!/usr/bin/env bash
###################################################################################
# Register new hooks in SuperGovernor on Base (chain 8453)
# Hooks to register are the new deployments from feat/hook-sizing-manifest branch.
#
# Usage: ./script/run/register_hooks_base.sh [simulate|execute] [account]
...
# New hooks deployed on Base in this branch (vs dev)
declare -A HOOKS
...
HOOKS["ApproveAndSwapOdosV3Hook"]="0x39a99991c7247E71D7148Db529e720D963103B7F"
...
HOOKS["StargateAdapterV2"]="0x0d9D81c5b1a8F18BAC35DA6731e80f0425954455"
HOOKS["Swap1InchHook"]="0xeFA058564f85408D994fC60164e7e32290352D6D"
HOOKS["SwapKyberSwapHook"]="0x05c49e05bb8575afdf1142cC95dA6747b069174A"
HOOKS["SwapOdosV3Hook"]="0xA987e92641A5677A65EFAa3c87232b41F3E367ca"
...
if [ -z "${BASE_MAINNET:-}" ]; then
    echo "Loading Base RPC from 1Password..."
    export BASE_MAINNET=$(op read op://5ylebqljbh3x6zomdxi3qd7tsa/BASE_RPC_URL/credential 2>/dev/null | tr -d '\n')
fi

if [ -z "${BASE_MAINNET:-}" ]; then
    echo "❌ BASE_MAINNET not set. Export it or ensure 1Password CLI is available."
    exit 1
fi
...
echo "============================================================"
echo "  Register Hooks in SuperGovernor — Base (chain $CHAIN_ID)"
...
if [[ "$MODE" == "execute" ]]; then
    echo "⚠️  EXECUTE MODE: This will send real transactions on Base."

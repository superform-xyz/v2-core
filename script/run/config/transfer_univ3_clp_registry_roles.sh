...
###################################################################################
# Transfer UniV3CLPRegistry Roles
###################################################################################
# Description:
#   Transfers UniV3CLPRegistry roles from DEPLOYER to governance addresses
#   across all chains in the target environment. Reads the registry address from
#   script/output/<env>/<chainId>/<ChainName>-latest.json per chain.
#
#   Role mapping:
#     - POSITION_MANAGER_ROLE -> GOVERNOR        (0x9e01...48eC)
#     - DEFAULT_ADMIN_ROLE    -> SUPER_GOVERNOR   (0x8922...ad2e)
#   Then revokes all roles from DEPLOYER.
#
# Usage:
#   ./script/run/config/transfer_univ3_clp_registry_roles.sh <environment> <mode> <account> [--slow] [--legacy] [--check-only]
#
# Arguments:
...
#   ./script/run/config/transfer_univ3_clp_registry_roles.sh prod simulate v2
#
#   # Execute role transfer on all prod chains
#   ./script/run/config/transfer_univ3_clp_registry_roles.sh prod execute deployer
#
#   # Check current role status only
#   ./script/run/config/transfer_univ3_clp_registry_roles.sh prod simulate v2 --check-only
###################################################################################
...
print_header() {
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${WHITE}      UniV3CLPRegistry Role Transfer Script                                ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
}
...
# ===== RESOLVE REGISTRY ADDRESSES PER CHAIN =====

print_separator
echo -e "${BLUE}Resolving UniV3CLPRegistry addresses from ${OUTPUT_DIR}...${NC}"

# Helper: read UniV3CLPRegistry address from chain's JSON output
...
for network_def in "${NETWORKS[@]}"; do
    ...
    addr=$(get_registry_address "$network_id" 2>/dev/null) || addr=""
    if [[ -n "$addr" ]]; then
        CHAIN_REGISTRY["$network_id"]="$addr"
...
if [[ $chains_found -eq 0 ]]; then
    echo -e "${RED}No registry addresses found. Nothing to do.${NC}"
    exit 1
fi
...
# Forge script path
SCRIPT="script/TransferUniV3CLPRegistryRoles.s.sol:TransferUniV3CLPRegistryRoles"

# Determine which sig to use
if [ "$CHECK_ONLY" = true ]; then
    FORGE_SIG="runCheck(uint256,uint64,address)"
else
    FORGE_SIG="run(uint256,uint64,address)"
fi
...
if [ "$MODE" = "execute" ] && [ "$CHECK_ONLY" = false ]; then
    print_separator
    echo -e "${WHITE}This will transfer UniV3CLPRegistry roles on $chains_found $ENVIRONMENT chain(s).${NC}"
    echo -e "${WHITE}  POSITION_MANAGER_ROLE -> GOVERNOR        (0x9e01f41da2212C1FBc32A041CfAEF72479FA48eC)${NC}"
...
for network_def in "${NETWORKS[@]}"; do
    ...
    # Skip if no registry address found for this chain
    registry_addr="${CHAIN_REGISTRY[$network_id]:-}"
    if [[ -z "$registry_addr" ]]; then
        echo -e "${YELLOW}Skipping $network_name ($network_id) - no registry address in output${NC}"
    ...
    print_network_header "$network_name ($network_id) @ $registry_addr"

    # Build forge command
    FORGE_CMD="forge script $SCRIPT \
        --sig \"$FORGE_SIG\" \
        $FORGE_ENV $network_id $registry_addr \
        --rpc-url ${!rpc_var} \
        --chain $network_id"
...
if [ "$CHECK_ONLY" = true ]; then
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║${WHITE}    UniV3CLPRegistry $ENVIRONMENT Role Check Complete!                          ${GREEN}║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
else
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║${WHITE}    UniV3CLPRegistry $ENVIRONMENT Role Transfer $MODE Complete!                  ${GREEN}║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
fi

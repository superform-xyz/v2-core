...
check_v2_addresses_on_network() {
    ...
    if [[ -n "$relevant_output" ]]; then
        log "INFO" "  📋 Contract Deployment Status:"
    ...
    if [[ -n "$availability_info" ]]; then
        log "INFO" "  📊 Contract Availability Analysis:"
...
preview_demo_deployment_diff() {
    ...
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${WHITE}                        📋 Demo Branch Deployment Preview 📋                         ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    for network in 1 8453 10; do
        ...
        echo -e "${CYAN}📍 Checking $network_slug (Chain ID: $network)...${NC}"
        ...
        if check_result=$(check_v2_addresses_on_network "$network" "$network_slug" "$admin_rpc"); then
            ...
            if [ "$deployed_count" -lt "$total_count" ]; then
                ...
                echo -e "  ${YELLOW}⚠️  $missing_count contracts need deployment (${deployed_count}/${total_count} already deployed)${NC}"
                has_changes=true
            else
                echo -e "  ${GREEN}✅ All contracts already deployed (${deployed_count}/${total_count})${NC}"
            fi
            
            # Show redeploy impact
            if [ "$REDEPLOY_FLAG" = "redeploy" ]; then
                echo -e "  ${YELLOW}🔄 REDEPLOY MODE: All contracts will get NEW addresses (timestamp salt)${NC}"
                has_changes=true
            else
                echo -e "  ${CYAN}🔒 FIXED SALT MODE: Contracts will use consistent addresses${NC}"
            fi
        else
    ...
    if [ "$has_changes" = true ] || [ "$missing_total" -gt 0 ]; then
        echo -e "${YELLOW}⚠️  Changes detected - deployment will modify or add contracts${NC}"
        log "DEBUG" "preview_demo_deployment_diff returning 0 (true) - changes detected"
        return 0
    else
        echo -e "${CYAN}ℹ️  All contracts already deployed with expected addresses${NC}"
...
show_demo_s3_diff() {
    ...
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${WHITE}                       📤 Demo Branch S3 Upload Preview 📤                            ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
        ...
        if [ -f "$contracts_file" ]; then
            ...
            if new_contracts=$(echo "$new_contracts" | jq -c '.' 2>/dev/null); then
                ...
                if has_contract_changes "$existing_contracts" "$new_contracts"; then
                    ...
                    echo -e "${CYAN}📍 $network_slug: No changes detected${NC}"
                fi
            else
    ...
    if [ "$has_changes" = false ]; then
        echo -e "${CYAN}ℹ️  No contract changes to upload to S3${NC}"
    fi

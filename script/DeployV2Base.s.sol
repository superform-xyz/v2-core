...
abstract contract DeployV2Base is Script, ConfigBase {
    ...
    /// @param env Environment (1 = vnet/dev, 0/2 = prod/staging)
    /// @return bytecodeDir Directory path for bytecode artifacts
    function __getBytecodeDirectory(uint256 env) internal pure returns (string memory) {
        if (env == 1) {
    ...
    function __getBytecodeArtifactPath(string memory contractName, uint256 env) internal pure returns (string memory) {
        return string(abi.encodePacked(__getBytecodeDirectory(env), contractName, ".json"));
    }

...
contract TestVaultBank is VaultBank {
    ...
    function exposed_retrieveSuperPosition(uint64 srcChainId, address srcTokenAddress, string calldata name, string calldata symbol, uint8 decimals) external returns (address) {
...
contract VaultBankTest is Helpers {
    ...
    function test_lockAsset_Success() public {
        ...
        assertEq(
            vaultBank.viewLockedAmount(user, address(token), destinationChainId),
            lockAmount,
        ...
        assertEq(
            ...
            lockAmount,
            "Total locked amount incorrect"
        );
        assertEq(
            vaultBank.viewAllLockedAssets(user, destinationChainId).length,
            1,
            "Locked assets length incorrect"
        );
        assertEq(
            vaultBank.viewAllLockedAssets(user, destinationChainId)[0],
            address(token),
            "Token not in locked assets"
        );
        assertEq(
            token.balanceOf(address(vaultBank)),
            lockAmount,
            "VaultBank balance incorrect"
        );
    ...
    function test_unlockAsset_InvalidProofAmount() public {
        ...
        mockProver.setValidateEventReturn(
            ...
            address(vaultBank),
            new bytes(128), 
            abi.encode(lockAmount - 1, CURRENT_CHAIN_ID, 0)
        );
    ...
    function test_unlockAsset_InvalidProofTargetedChain() public {
        ...
        mockProver.setValidateEventReturn(
            ...
            address(vaultBank),
            new bytes(128),
            abi.encode(lockAmount, CURRENT_CHAIN_ID + 1, 0)
        );
    ...
    function test_unlockAsset_Success() public {
        ...
        assertEq(
            ...
            "Vault balance after unlock incorrect"
        );
        assertEq(
            vaultBank.viewLockedAmount(user, address(token), DST_CHAIN_ID),
            lockAmount - unlockAmount,
    ...
    function test_distributeSuperPosition_InvalidProofEmitter() public {
        ...
        bytes memory mockUnindexedData = abi.encode(
            amount,
            DST_CHAIN_ID,
            uint64(block.chainid),
            uint256(0) 
    ...
    function test_distributeSuperPosition_Success() public {
        ...
        vm.mockCall(
            address(vaultBank),
            abi.encodeWithSignature("_retrieveSuperPosition(uint64,address,string,string,uint8)", 
            DST_CHAIN_ID, address(token), "Test Token", "TT", 18),
            abi.encode(superPositionAddress)
        );
        
        vm.mockCall(
            address(vaultBank),
            abi.encodeWithSignature("_mintSP(address,address,uint256)", 
            account, superPositionAddress, amount),
            abi.encode()
        );
    ...
    function test_distributeSuperPosition_Success_ExistingTokenToSuperposition() public {
        ...
        vm.mockCall(
            address(vaultBank),
            abi.encodeWithSignature("_mintSP(address,address,uint256)", 
            account, existingSPAddress, amount),
            abi.encode()
        );
    ...
    function test_burnSuperPositions_SuperPositionNotFound() public {
        ...
        vm.mockCall(
            address(vaultBank),
            abi.encodeWithSignature("_spAssets(address)", nonExistentSP),
            abi.encode(false) 
        );
    ...
    function test_viewLockedAmount() public {
        ...
        assertEq(vaultBank.viewLockedAmount(user, address(token), DST_CHAIN_ID), lockAmount, "Locked amount should match the locked amount");
    }
    ...
    function test_viewTotalLockedAsset() public {
        ...
        assertEq(vaultBank.viewTotalLockedAsset(user, address(token)), lockAmount1 + lockAmount2, "Total locked amount should match sum of all locks");
    }
    ...
    function test_getSuperPositionForAsset() public {
        ...
        assertEq(vaultBank.getSuperPositionForAsset(mockChainId, mockToken), address(0), "Should return zero address initially");
        
        vaultBank.exposed_setTokenToSuperPosition(mockChainId, mockToken, mockSuperPosition);
        
        assertEq(vaultBank.getSuperPositionForAsset(mockChainId, mockToken), mockSuperPosition, "Should return correct super position address");
    }
    ...
    function test_getAssetForSuperPosition() public {
        ...
        assertEq(vaultBank.getAssetForSuperPosition(mockChainId, mockSuperPosition), address(0), "Should return zero address initially");
        
        vaultBank.exposed_setSuperPositionToToken(mockSuperPosition, mockChainId, mockToken);
        
        assertEq(vaultBank.getAssetForSuperPosition(mockChainId, mockSuperPosition), mockToken, "Should return correct token address");
    }
    ...
    function test_retrieveSuperPosition_NewSP() public {
        ...
        assertEq(vaultBank.getSuperPositionForAsset(mockChainId, mockToken), retrievedSP, "Should set token to SP mapping");
        assertEq(vaultBank.getAssetForSuperPosition(mockChainId, retrievedSP), mockToken, "Should set SP to token mapping");
    }
    ...
    function test_batchDistributeRewardsToSuperBank() public {
        ...
        vm.mockCall(
            address(superGovernor),
            abi.encodeWithSignature("isSuperBank(address)", mockSuperBank),
            abi.encode(true)
        );

        vm.mockCall(
            address(superGovernor),
            abi.encodeWithSignature("isRelayer(address)", address(this)),
            abi.encode(true)
        );

    ...
    InternalHelpers
{
    ...
    function setUp() public virtual {
        ...
        // Deploy persistent MockRegistry for consistent account generation across all forks
        persistentMockRegistry = new MockRegistry();
        vm.makePersistent(address(persistentMockRegistry));
    ...
    function _deployHooks(Addresses[] memory A) internal returns (Addresses[] memory) {
        ...
        for (uint256 i = 0; i < chainIds.length; ++i) {
            ...
            A[i].swap0xHook = new Swap0xV2Hook{ salt: SALT }(ALLOWANCE_HOLDER);
            vm.label(address(A[i].swap0xHook), SWAP_0X_HOOK_KEY);
            hookAddresses[chainIds[i]][SWAP_0X_HOOK_KEY] = address(A[i].swap0xHook);
            hooks[chainIds[i]][SWAP_0X_HOOK_KEY] =
                Hook(SWAP_0X_HOOK_KEY, HookCategory.Swaps, HookCategory.TokenApprovals, address(A[i].swap0xHook), "");
            hooksByCategory[chainIds[i]][HookCategory.Swaps].push(hooks[chainIds[i]][SWAP_0X_HOOK_KEY]);
            hooksAddresses[15] = address(A[i].swap0xHook);
        ...
        returns (bytes memory, address)
    {
        ...
        // Use persistent MockRegistry to ensure consistent account generation across calls
        bytes memory initData = INexusBootstrap(p.nexusBootstrap).getInitNexusCalldata(
            validators, executors, hook, fallbacks, IERC7484(persistentMockRegistry), attesters, threshold
        );

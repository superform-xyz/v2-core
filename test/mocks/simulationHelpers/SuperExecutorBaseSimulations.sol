...
abstract contract SuperExecutorBaseSimulations is ERC7579ExecutorBase, ISuperExecutor, ReentrancyGuard {
    ...
    address public ledgerConfiguration;
    ...
    constructor(address superLedgerConfiguration_) {
        ledgerConfiguration = superLedgerConfiguration_;
    }
    ...
    function _updateAccounting(address account, address hook, bytes memory hookData) internal virtual {
        ...
        if (_type == ISuperHook.HookType.INFLOW || _type == ISuperHook.HookType.OUTFLOW) {
            ...
            ISuperLedgerConfiguration.YieldSourceOracleConfig memory config =
                ISuperLedgerConfiguration(ledgerConfiguration).getYieldSourceOracleConfig(yieldSourceOracleId);

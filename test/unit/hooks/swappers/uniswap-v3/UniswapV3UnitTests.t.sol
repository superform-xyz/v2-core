...
contract UniswapV3HookTest is Helpers {
    ...
    function setUp() public {
        ...
        swapHook = new SwapUniswapV3Hook(address(router));
        approveAndSwapHook = new ApproveAndSwapUniswapV3Hook(address(router));
    }

    /*//////////////////////////////////////////////////////////////
                         SwapUniswapV3Hook Tests
    //////////////////////////////////////////////////////////////*/
    ...
    /*//////////////////////////////////////////////////////////////
                    ApproveAndSwapUniswapV3Hook Tests
    //////////////////////////////////////////////////////////////*/
    ...
    function test_ApproveAndSwapHook_Constructor_RevertIf_AddressZero() public {
        vm.expectRevert(BaseHook.ADDRESS_NOT_VALID.selector);
        new ApproveAndSwapUniswapV3Hook(address(0));
    }
    ...
    function test_SwapHook_SlippageRecalculation_VerifyValues() public {
        ...
        // tokenIn (32 bytes), tokenOut (32 bytes), fee (32 bytes), recipient (32 bytes),
        // deadline (32 bytes), amountIn (32 bytes), amountOutMinimum (32 bytes), sqrtPriceLimitX96 (32 bytes)
    ...
    function test_SwapUniV3_DecodeAmounts() public view {
        bytes memory data = _buildHookData(false);
        assertEq(swapHook.decodeAmounts(data)[0], originalAmountIn);
    }

    function test_SwapUniV3_ReplaceCalldataAmounts() public view {
        bytes memory data = _buildHookData(false);
    ...
    function testFuzz_SwapUniV3_ReplaceCalldataAmounts(uint256 fuzzAmount) public view {
    ...
    function test_ApproveAndSwapUniV3_DecodeAmounts() public view {
        bytes memory data = _buildHookData(false);
        assertEq(approveAndSwapHook.decodeAmounts(data)[0], originalAmountIn);
    }

    function test_ApproveAndSwapUniV3_ReplaceCalldataAmounts() public view {
        bytes memory data = _buildHookData(false);
        uint256 newAmount = 2e18;
    ...
    function testFuzz_ApproveAndSwapUniV3_ReplaceCalldataAmounts(uint256 fuzzAmount) public view {
        vm.assume(fuzzAmount > 0);
        bytes memory data = _buildHookData(false);
    ...
    function test_SwapUniV3_ReplaceCalldataAmounts_ThenBuild() public view {
        bytes memory data = _buildHookData(false);
    ...
    function test_ApproveAndSwapUniV3_ReplaceCalldataAmounts_ThenBuild() public view {
        bytes memory data = _buildHookData(false);
        uint256 newAmount = 500;
    ...
    function test_SwapUniV3_ReplaceCalldataAmounts_PreservesOtherFields() public view {
        bytes memory data = _buildHookData(false);
        bytes memory replaced = swapHook.replaceCalldataAmounts(data, _singleAmount(999));
        assertEq(replaced.length, data.length);
        // AMOUNT_POSITION is 92 (Layer 1 standard offset)
    ...
    function _buildHookData(bool usePrevHookAmount) internal view returns (bytes memory) {
        return bytes.concat(
            ...
            bytes20(tokenOut),                             // [72:92] outputToken
    ...
    function test_SwapHook_ZeroAmountIn() public view {
        ...
        assertEq(decodedAmountOutMinimum, 0);
    }
    ...
    function test_SwapHook_RecipientForcedToAccount() public view {
        // Recipient is always forced to account — not a field in the hook data
        bytes memory data = _buildHookData(false);
    ...
    function testFuzz_SwapHook_DataLength(uint8 extraBytes) public view {
        // Test with various data lengths >= 289

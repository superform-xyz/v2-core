...
contract EthenaHooksTests is Helpers {
    ...
    /*//////////////////////////////////////////////////////////////
                          DECODE AMOUNT TESTS
    //////////////////////////////////////////////////////////////*/
    function test_EthenaCooldownSharesHook_DecodeAmounts() public view {
        bytes memory data = _encodeCooldownData(false);
        uint256 decodedAmount = cooldownSharesHook.decodeAmounts(data)[0];
        assertEq(decodedAmount, amount);
    }

    function test_EthenaCooldownSharesHook_ReplaceCalldataAmounts() public view {
        bytes memory data = _encodeCooldownData(false);
        uint256 newAmount = 2e18;
        bytes memory result = cooldownSharesHook.replaceCalldataAmounts(data, _singleAmount(newAmount));
        assertEq(result.length, data.length);
        assertEq(cooldownSharesHook.decodeAmounts(result)[0], newAmount);
    }

    function testFuzz_EthenaCooldownSharesHook_ReplaceCalldataAmounts(uint256 fuzzAmount) public view {
        vm.assume(fuzzAmount > 0);
        bytes memory data = _encodeCooldownData(false);
        bytes memory result = cooldownSharesHook.replaceCalldataAmounts(data, _singleAmount(fuzzAmount));
        assertEq(cooldownSharesHook.decodeAmounts(result)[0], fuzzAmount);
    }
    ...
    function test_EthenaUnstakeHook_PrePostExecute() public {
        ...
        assertEq(
            unstakeHook.getOutAmount(address(this)),
            usde.balanceOf(address(this)),
            "OutAmount should be initial balance"
        );
        ...
        assertEq(
            unstakeHook.getOutAmount(address(this)),
            amount * 2 - expectedOutAmount,
            "OutAmount should be updated correctly"
        );
    ...
    function test_EthenaCooldown_ReplaceCalldataAmounts_ThenBuild() public view {
        bytes memory data = _encodeCooldownData(false);
        uint256 newAmount = 500;
        bytes memory replaced = cooldownSharesHook.replaceCalldataAmounts(data, _singleAmount(newAmount));
        Execution[] memory executions = cooldownSharesHook.build(address(0), address(this), replaced);
        assertEq(executions.length, 3);
        assertEq(cooldownSharesHook.decodeAmounts(replaced)[0], newAmount);
    }

...
contract MorphoBorrowHookTest is BaseTest {
    ...
    function test_Build_RevertIf_ZeroAmount() public {
        ...
        hook.build(
            ...
            address(this),
            abi.encodePacked(
                address(loanToken), address(collateralToken), address(oracle), irm, uint256(0), lltv, false, false
            )
        );
    }
    ...
    function _encodeData(bool usePrevHook) internal view returns (bytes memory) {
        return abi.encodePacked(
            address(loanToken), address(collateralToken), address(oracle), irm, amount, lltv, usePrevHook, false
        );
    }
}

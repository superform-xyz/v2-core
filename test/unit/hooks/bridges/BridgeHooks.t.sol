...
contract BridgeHooks is Helpers {
    ...
    function test_CancelOrderHook_Build() public {
        ...
        assertEq(executions.length, 1);
        assertEq(executions[0].target, address(this)); // Should be the dlnSource address
        assertEq(executions[0].value, 0.1 ether); // Using mockValue
        ...
        assertTrue(executions[0].callData.length > 0, "CallData should not be empty");
    }
    ...
    function test_CancelOrderHook_PreExecute() public {
        cancelOrderHook.preExecute(address(0), address(0), "");
    }

    function test_CancelOrderHook_PostExecute() public {
        cancelOrderHook.postExecute(address(0), address(0), "");
    }

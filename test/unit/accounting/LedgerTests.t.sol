...
contract LedgerTests is Helpers {
    ...
    /*//////////////////////////////////////////////////////////////
                    PREVIEW OUTFLOW WITHOUT FEES TESTS
    //////////////////////////////////////////////////////////////*/

    struct PreviewTestConfig {
        bytes4 oracleId;
        address oracle;
        uint256 feePercent;
    ...
    function test_PreviewOutflowWithoutFees_NormalCase() public {
        PreviewTestConfig memory testConfig = PreviewTestConfig({
            oracleId: bytes4(keccak256("test")),
        ...
        PreviewTestParams memory testParams = PreviewTestParams({
            user: address(0x456),
            yieldSource: address(0x789),
            assetOut: address(0xABC),
            initialShares: 1000e18,
            usedShares: 500e18,
            assetOutput: 1100e18 // 10% profit over cost basis
         });
        ...
        ISuperLedgerConfiguration.YieldSourceOracleConfigArgs[] memory configs =
            new ISuperLedgerConfiguration.YieldSourceOracleConfigArgs[](1);
        configs[0] = ISuperLedgerConfiguration.YieldSourceOracleConfigArgs({
            yieldSourceOracleId: testConfig.oracleId,
            yieldSourceOracle: testConfig.oracle,
            feePercent: testConfig.feePercent,
            feeRecipient: testConfig.feeRecipient,
            ledger: testConfig.ledger
        });
        config.setYieldSourceOracles(configs);
        ...
        mockBaseLedger.updateAccounting(
            testParams.user,
            testParams.yieldSource,
            testConfig.oracleId,
            true, // isInflow
            testParams.initialShares,
            0
        );

        // Mock getAssetOutput call
        vm.mockCall(
            testConfig.oracle,
            abi.encodeWithSignature(
                "getAssetOutput(address,address,uint256)", testParams.yieldSource, address(0), testParams.usedShares
            ),
            abi.encode(testParams.assetOutput)
        );

        // Test previewOutflowWithoutFees
        uint256 netAmount = mockBaseLedger.previewOutflowWithoutFees(
            testConfig.oracleId,
            testParams.yieldSource,
            testParams.assetOut,
            testParams.user,
            testParams.usedShares,
            testConfig.feePercent
        );

        // Cost basis for half the shares = 500e18
        // Profit = 1100e18 - 500e18 = 600e18
        // Fee = 600e18 * 10% = 60e18
        // Net amount = 1100e18 - 60e18 = 1040e18
        uint256 expectedNetAmount = testParams.assetOutput - (600e18 * testConfig.feePercent / 10_000);
        assertEq(netAmount, expectedNetAmount, "Net amount calculation incorrect");
    }

    function test_PreviewOutflowWithoutFees_NoProfit() public {
        PreviewTestConfig memory testConfig = PreviewTestConfig({
            oracleId: bytes4(keccak256("test")),
            oracle: address(mockOracle),
            feePercent: 1000, // 10%
        ...
        PreviewTestParams memory testParams = PreviewTestParams({
            ...
            initialShares: 1000e18,
            usedShares: 500e18,
            assetOutput: 500e18 // No profit, equals cost basis
         });
        ...
        // No profit means no fees, so net amount equals asset output
        assertEq(netAmount, testParams.assetOutput, "Net amount should equal asset output when no profit");
    }

    function test_PreviewOutflowWithoutFees_ZeroFeesWithProfit() public {
        PreviewTestConfig memory testConfig = PreviewTestConfig({
            oracleId: bytes4(keccak256("test")),
            oracle: address(mockOracle),
            feePercent: 0, // 0%
        ...
        // Test previewOutflowWithoutFees - should revert when there's profit but fee percent is 0
        vm.expectRevert(ISuperLedgerData.FEE_NOT_SET.selector);
        mockBaseLedger.previewOutflowWithoutFees(
            testConfig.oracleId,
            testParams.yieldSource,
            testParams.assetOut,
            testParams.user,
    ...
    function test_PreviewOutflowWithoutFees_ZeroFeesNoProfit() public {
        PreviewTestConfig memory testConfig = PreviewTestConfig({
            oracleId: bytes4(keccak256("test")),
            oracle: address(mockOracle),
            feePercent: 0, // 0%
        ...
        // Test previewOutflowWithoutFees - should work when no profit (no fees calculated)
        uint256 netAmount = mockBaseLedger.previewOutflowWithoutFees(
            testConfig.oracleId,
            testParams.yieldSource,
            testParams.assetOut,
        ...
        assertEq(
            netAmount,
            testParams.assetOutput,
            "Net amount should equal asset output when no profit and fee percent is 0"
        );
    }

    function test_PreviewOutflowWithoutFees_InvalidOracle() public {
        PreviewTestParams memory testParams = PreviewTestParams({
            user: address(0x456),
            yieldSource: address(0x789),
            assetOut: address(0xABC),
            initialShares: 0, // not used
            usedShares: 500e18,
            assetOutput: 0 // not used
         });

        bytes4 invalidOracleId = bytes4(keccak256("invalid"));
        uint256 feePercent = 1000;

        // Try to call with non-existent oracle config
    ...
    function test_PreviewOutflowWithoutFees_MaxFeePercent() public {
        PreviewTestConfig memory testConfig = PreviewTestConfig({
            oracleId: bytes4(keccak256("test")),
            oracle: address(mockOracle),
            feePercent: 5000, // 50% (max allowed)
            feeRecipient: address(this),
            ledger: address(mockBaseLedger)
        });
        ...
        PreviewTestParams memory testParams = PreviewTestParams({
            ...
            assetOut: address(0xABC),
            initialShares: 1000e18,
            usedShares: 1000e18, // Use all shares
            assetOutput: 2000e18 // 100% profit
         });
        ...
        // Net amount = 2000e18 - 500e18 = 1500e18
        uint256 expectedNetAmount = testParams.assetOutput - (1000e18 * testConfig.feePercent / 10_000);
        assertEq(netAmount, expectedNetAmount, "Net amount calculation incorrect with max fee percent");
    }
    ...
    function test_ProposeYieldSourceOracleConfig_NewProposalAfterExpiration() public {
        ...
        initialConfig.oracleId = bytes4(keccak256("test"));
        ...
        bytes4[] memory oracleIdsToAccept = new bytes4[](1);

...
abstract contract CrosschainTestsGateway is Helpers, RhinestoneModuleKit, InternalHelpers, MultichainTestUtils {
    ...
    address public constant GATEWAY_WALLET_ADDR = 0x0077777d7EBA4688BDeF3E311b846F25870A19B9;
    address public constant GATEWAY_MINTER_ADDR = 0x0022222ABE238Cc2C7Bb1f21003F0a260052475B;
    ...
    function _setupChain(ChainConfig memory chainConfig) internal {
        ...
        superDestinationExecutor = address(new SuperDestinationExecutor(address(ledgerConfig)));
    }
    ...
    ) internal view returns (TransferSpec memory transferSpec) {
        // Create hookData for SuperDestinationExecutor call
        bytes memory hookData = _createHookDataForDestinationExecution(destChain, amount, recipientAddr);
        
        transferSpec = TransferSpec({
            version: 1,
            sourceDomain: sourceChain.domain,
            destinationDomain: destChain.domain,
            sourceContract: bytes32(uint256(uint160(GATEWAY_WALLET_ADDR))),
            destinationContract: bytes32(uint256(uint160(GATEWAY_MINTER_ADDR))),
            ...
            destinationRecipient: bytes32(uint256(uint160(superDestinationExecutor))), // Send to SuperDestinationExecutor
    ...
    function _createHookDataForDestinationExecution(
        ...
        // Create hooks for destination execution (approve + deposit to vault)
        address[] memory hooksAddresses = new address[](2);
        hooksAddresses[0] = address(circleGatewayMinterHook); // First mint from Circle Gateway
        hooksAddresses[1] = address(approveAndDeposit4626Hook); // Then deposit to vault

        bytes[] memory hooksData = new bytes[](2);
        // Mock minter hook data (would normally contain attestation + signature)
        hooksData[0] = ""; // Empty for now, would be filled by Circle Gateway
        // Vault deposit hook data
        hooksData[1] = abi.encodePacked(
            bytes32(bytes("ERC4626YieldSourceOracle")), // yieldSourceOracleId
            address(mockVault),                         // yieldSource
            destChain.usdc,                            // token
            amount,                                    // amount
    ...
    // Note: Burn intent and attestation signing methods are inherited from MultichainTestUtils

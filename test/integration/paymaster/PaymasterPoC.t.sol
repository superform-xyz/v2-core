...
contract PoC is PaymasterHelper {
    ...
    function test_extraGas() public {
        ...
        data.entry = ISuperExecutor.ExecutorEntry({ hooksAddresses: data.hooksAddresses, hooksData: data.hooksData });
        ...
        IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).depositTo{
            value: data.maxGasLimit * data.maxFeePerGas + 5e15
        }(address(data.paymaster));

        // we prank paymaster because we want to use the actual gas in entryPoint.handleOps(). without the gas in the
        // wrapper
        ...
        IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).handleOps{ gas: data.maxGasLimit }(
            data.ops, payable(address(this))
        );

        uint256 actualGas = vm.stopSnapshotGas();

        // the real gas cost
        // we add 10e6 because EntryPoint adds verificationGasLimit to gasUsed value in its calculations internally and
        ...
        uint256 whatShouldBeRefunded =
            data.paymaster.calculateRefund(data.maxGasLimit, data.maxFeePerGas, realGasCost, 0);
    ...
    function test_incorrectGas() public {
        ...
        SuperNativePaymaster paymaster =
            new SuperNativePaymaster(IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032));
        ...
        bytes memory paymasterAndData =
            abi.encodePacked(address(paymaster), paymasterVerificationGasLimit, paymasterPostOpGasLimit, paymasterData);
        ...
        IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).depositTo{ value: maxGasLimit * maxFeePerGas + 5e15 }(
            address(paymaster)
        );
        ...
        // beneficiary is this as it does not matter for gas estimations
        IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).handleOps{ gas: maxGasLimit }(
            ops, payable(address(this))
        );
    ...
    function test_postOpGasMultiplier() public {
        ...
        IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).depositTo{ value: initialDeposit }(
            address(data.paymaster)
        );
        uint256 depositBefore =
            IEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032).getDepositInfo(address(data.paymaster)).deposit;
    ...
    function test_refundDOS() public {
        ...
        // SuperBundler calls paymaster.handleOps() with maxGasLimit * maxFeePerGas ether value, as was paid by the
        // account

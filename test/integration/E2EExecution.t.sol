...
contract E2EExecutionTest is MinimalBaseNexusIntegrationTest {
    ...
    function _buildUserOp(
        ...
        testData.hooksData[0] = _createApproveHookData(token, address(acrossHook), amount, false);
    ...
    function _buildDestinationMessage(address token, uint256 amount)
        internal
        pure
    ...
    function _getSignatureData(
        ...
        address entryPoint
    ) internal returns (bytes memory sigData, bytes32[][] memory proof, bytes32 root, bytes memory signature) {

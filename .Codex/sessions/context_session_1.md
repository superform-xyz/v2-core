...
- Updated the PR body with explicit integration/release instructions: use `abi.encode(routeKind, deadline, packedPath)`; obtain the swap path from pinned `sugar-sdk v0.4.2` `quote.input.route_for_swap.encoded` rather than quoter `route.encoded`; reject mixed routes and unknown CL factories; keep native ETH, exact-output, Permit2-signature, and arbitrary-command flows out of scope; do not replace router `0xcAF22...` without repeating source/runtime/fork verification; require the 104-contract/two-missing-address staging preflight; and promote production locks only after staging verification and security sign-off.

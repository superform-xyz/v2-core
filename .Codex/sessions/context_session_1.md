...
- Revised the PR instructions after user feedback to be specific to reviewing and releasing this diff. The reviewer notes now cover the Base-only/pinned-router invariant, intentional six-WithId and adapter-count deployment corrections, unchanged slots `0..77` plus new slots `78/79`, development-versus-production artifact boundary, expected 104-contract/two-missing-address preflight, and the high-risk code areas reviewers should inspect. General OMS/Sugar integration guidance was removed from the PR description.

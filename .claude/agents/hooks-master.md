...
**NatSpec Data Layout Documentation**:

**MANDATORY 52-byte Strategy Header**:
ALL hooks MUST start with a 52-byte strategy header. The first two fields are ALWAYS:
- `placeholder0` (uint256, 32 bytes, offset 0) — reserved for `yieldSourceOracleId` when used by vault hooks
- `placeholder1` (address, 20 bytes, offset 32) — reserved for `yieldSource` when used by vault hooks

The naming convention depends on whether the hook uses these fields:

| Case | Field at offset 0 | Field at offset 32 | Example hooks |
|------|-------------------|-------------------|---------------|
| **Both used** (vault hooks) | `bytes32 yieldSourceOracleId` | `address yieldSource` | Deposit4626VaultHook, Redeem4626VaultHook |
| **Only yieldSource used** | `uint256 placeholder0` | `address yieldSource` (or domain-specific name) | YearnClaimOneRewardHook, FluidClaimRewardHook |
| **Neither used** (non-vault hooks) | `uint256 placeholder0` | `address placeholder1` | TransferHook, MorphoBorrowHook, bridge hooks |

**IMPORTANT**: `placeholder` is a reserved keyword prefix. Any unused/reserved field in the data layout MUST be named `placeholder0`, `placeholder1`, `placeholder2`, etc. (incrementing). Never use bare `placeholder` — always use the numbered form.

Hook-specific fields always start at offset 52.

**Example: Non-vault hook (neither header field used)**:
```solidity
...
**Example: Vault hook (both header fields used)**:
```solidity
...
**Example: Hook with only yieldSource used (oracleId is placeholder)**:
```solidity
/// @dev data has the following structure
/// @notice         uint256 placeholder0 = BytesLib.toUint256(data, 0);
/// @notice         address yieldSource = BytesLib.toAddress(data, 32);
...
**Example: Complex encoding with dynamic data (bridge hook)**:
```solidity
/// @dev data has the following structure (standard 52-byte strategy header + hook-specific):
/// @notice         uint256 placeholder0 = BytesLib.toUint256(data, 0);
/// @notice         address placeholder1 = BytesLib.toAddress(data, 32);
...
**Dynamic Data Length Fields — `_paramLength` Naming Convention**:

When a hook's data layout contains a length field that describes the size of a subsequent dynamic-length parameter, the NatSpec name MUST follow the pattern `{dynamicField}_paramLength`. This makes it immediately clear which dynamic field the length describes.
...
KEY RULE: must be in `pre-dev` branch! Check if you aren't and alert the user

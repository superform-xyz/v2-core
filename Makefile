...
coverage-genhtml :; FOUNDRY_PROFILE=coverage forge coverage --jobs 10 --ir-minimum --report lcov && genhtml lcov.info --branch-coverage --output-dir coverage --ignore-errors inconsistent,corrupt --exclude 'src/hooks/claim/gearbox/*' --exclude 'src/hooks/claim/yearn/*' --exclude 'src/hooks/claim/fluid/*' --exclude 'src/hooks/loan/morpho/*' --exclude 'src/hooks/stake/*' --exclude 'src/hooks/swappers/spectra/*' --exclude 'src/hooks/swappers/pendle/*' --exclude 'src/hooks/vaults/vault-bank/*' --exclude 'src/vendor/*' --exclude 'test/*'
...
test-vvv :; forge test --match-contract CrosschainTestsCentrifuge -vvv --jobs 10

test-integration :; forge test --match-test test_BASE_to_ETH_And_7540RequestDeposit -vvv --jobs 10

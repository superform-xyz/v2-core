...
contract SuperMerkleValidatorTest is BaseTest {
    ...
    function setUp() public override {
        ...
        dummyUserOp = instance.getExecOps(
            address(this), 0, abi.encodeWithSelector(IERC20.approve.selector, address(this), 1e18), address(instance.defaultValidator)
        );
    ...
    modifier givenTheSignatureIsValid(uint256 timestamp) {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(address(this), dummyUserOp.userOp.nonce, dummyUserOp.userOp.callData, dummyUserOp.userOp.accountGasLimits));
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(address(this), dummyUserOp.userOp.nonce, dummyUserOp.userOp.callData, dummyUserOp.userOp.accountGasLimits));

        bytes32 messageHash = keccak256(abi.encode(validator.namespace(), merkleRoot, leaf, address(this), dummyUserOp.userOp.nonce));
    ...
    function test_WhenTimestampIsExpired()
        external
        givenTheSignatureIsValid(block.timestamp - 1 hours)
    {
    ...
    function test_WhenTimestampIsValid()
        external
        givenTheSignatureIsValid(block.timestamp + 1 hours)
    {
        dummyUserOp.userOp.signature = validSigData;
    ...
    function test_ValidSignatureWithActualData(uint256 amount)
        external 
    {   
        // valid amount
        ...
        // merkle proof
        //  -- leaf
        bytes32 leaf = keccak256(abi.encodePacked(account, userOpData.userOp.nonce, userOpData.userOp.callData, userOpData.userOp.accountGasLimits));

        //  -- proof
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(account, userOpData.userOp.nonce, userOpData.userOp.callData, userOpData.userOp.accountGasLimits));
...

        {
            ...
            bytes32 messageHash = keccak256(abi.encode(validator.namespace(), merkleRoot, leaf, account, userOpData.userOp.nonce));
            bytes32 ethSignedMessageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", messageHash));

...
contract ApproveERC721Hook is BaseHook, ISuperHook {
    ...
    function build(bytes memory data) external pure override returns (Execution[] memory executions) {
        (address token, address spender, uint256 tokenId) = abi.decode(data, (address, address, uint256));

        if (token == address(0) || spender == address(0)) revert ADDRESS_NOT_VALID();

        executions = new Execution[](1);
        executions[0] =
            Execution({ target: token, value: 0, callData: abi.encodeCall(IERC721.approve, (spender, tokenId)) });
    }
}

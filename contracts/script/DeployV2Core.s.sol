...
contract DeployV2Core is Script {
    ...
    address public constant DEPLOYER = 0xd468A8f2f0CF8c905FC95dE67a94C1f8feaa3c8B;
    ...
    function deploy(uint64 chainId) public {
        ...
        deployer = Create3Deployer(DEPLOYER);

        address superRbac = deployer.deploy(
            keccak256("SUPER_RBAC_V1"), abi.encodePacked(type(SuperRbac).creationCode, abi.encode(ZERO))
        );

        address superRegistry = deployer.deploy(
            keccak256("SUPER_REGISTRY_V1"), abi.encodePacked(type(SuperRegistry).creationCode, abi.encode(ZERO))
        );
